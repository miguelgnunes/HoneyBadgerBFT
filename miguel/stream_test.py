...
def client_test_freenet(N, t, options):
    ...
    transaction_set = set([encodeMyTransaction(Transaction(j, receive_envelope())) for j in range(int(options.tx))])
    ...
    for i in range(N):
        ...
        controlChannels[i].put(('IncludeTransaction', transaction_set))
